...
We notice that each unit can be expanded only as needed, one symbol at a time. <span style='text-shadow:0 0 5px yellow'>In other words, a *unit* can be used as is, or decomposed into its constituents</span>.
...
D_y\mathbf{F} &=\mathbf{\color{lightgray}unit_z}\cdot
	\frac{\partial\mathbf{F}}{\partial y}
	\cdot\mathbf{\color{lightgray}unit_{y}}\\\\
D_x\mathbf{F} &=\mathbf{\color{lightgray}unit_z}\cdot
	\frac{\partial\mathbf{F}}{\partial y}
	\cdot\frac{\partial y}{\partial x}
	\cdot\mathbf{\color{lightgray}unit_{x}}\\\\
\vdots\\\\
D_a\mathbf{F} &=\mathbf{\color{lightgray}unit_z}\cdot
	\frac{\partial\mathbf{F}}{\partial y}
	\cdot\frac{\partial y}{\partial x}
	\cdot\quad\dots\quad
...
f'(y)
	&=\mathbf{\color{lightgray}unit_z}\cdot
...
We compute the components and partial derivatives at $x = 2$:
$$
\begin{align*}
...
f'(y)
	&=\mathbf{\color{lightgray}unit_z} \cdot
    \underbrace{\frac{\partial{z}}{\partial{y}}
    \cdot\mathbf{\color{lightgray}unit_y}
    }_{\color{blue}0.25}
		\\\\
f'(x)
	&=\mathbf{\color{lightgray}unit_z}\cdot 0.25 \cdot
    \underbrace{\frac{\partial{y}}{\partial{x}}
		\cdot
        \mathbf{\color{lightgray}unit_x}
    }_{\color{blue}4}
\end{align*}
$$
...
Essentially, in the standard chain rule, <span style='text-shadow: 0 0 5px yellow'>the evaluation of the total derivative is deferred to the very end</span>. Note that every line (starting from *then*) represents an expansion step. We recursively expand each symbol, finally plug the input $x$ in at the end and compute the expression. Because of this deferment, we ended up having to perform <strong><em>two branches</em></strong> of computations. We can easily trace this with a computation graph (left as an exercise for the reader -- you can check out the [previous post](/blog/2024-06-04-think-symbolically) for example graphs).

On the other hand, in the modified version, rather than deferring, we use the input $x$ right away and <span style='text-shadow: 0 0 5px yellow'>compute each partial derivative as soon as possible</span>. Whenever we evalute a function $f$, we also evaluate its partial derivative $f'$ at the respective symbolic unit. By the time we get to evaluating the total derivative, we *already* have the partial derivative quantities. We just need to plug in and complete the evaluation.
...
For example, we know that a running program is treated as a process by the OS and has resources allocated accordingly. The **unit** here is *process*, and the load on the system is the amount of processes running concurrently. That is one way of framing the measurement. On the other hand, we can also go deeper into the program's source code and map out the function call graph, each function as *sub-unit* and its resource allocation. In this case, we may frame our measurements in terms of the more detailed resource allocation.
