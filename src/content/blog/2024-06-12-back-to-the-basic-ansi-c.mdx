...
Modern hardware have moved on from 32-bit to 64-bit. Many kernel `struct` were expanded to accomodate more information about the system. Many default specs also changed from EXT2 to EXT4, such as block size and inode size. Ultimately, the mismatch in specs and memory layouts caused the program to spit out garbage.
...
I figured this part out when started reworking the project a few months ago. I realized that I was missing so much knowledge about the abstraction layer just below the general applications. Now I have an added layer of context from Ritchie and Kernighan's book. It helps connect many fragments in my head together.
