...
Let's start with a simple example. The [functional](https://en.wikipedia.org/wiki/Higher-order_function) below is commposed of other functions, whose exact expressions we don't care about yet. We only care about the high-leveled view for now.
$$
\mathbf{F}=f(g(x))
$$
...
This final operation yields a value of $1$.

However, it should be more accurately read as
...
The "box" can be thought of as a **unit** <u>relative</u> to the larger process.
